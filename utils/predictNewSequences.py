...
def parseArgument():
	...
	parser.add_argument("--swapLabels", action='store_true', required=False, help='Swap the negative and positive labels for evaluation')
...
def predictNewSequencesClassification(options, model, X, Y):
	...
	for i in range(Y.shape[1]):
		...
		if options.label == None:
			...
			# Print results
			print("loss: %f, accuracy: %f, sensitivity: %f, specificity: %f, auc: %f, precision: %f, NPV: %f, auprc: %f, auprg: %f, F1: %f, recallAt0.05FDR: %f, recallAt0.1FDR: %f" \
				% (loss, acc, sens, spec, AUC, precision, NPV, AUPRC, AUPRG, F1, recallAt05FDR, recallAt1FDR));
