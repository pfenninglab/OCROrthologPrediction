...
def parseArgument():
        ...
        parser.add_argument("--splitCharacterPeakList", required=False,\
                help='Character for splitting the peak list')
...
def filterPeakNames(options):
	...
	for line in peakListFile:
		...
		elif options.splitCharacterPeakList == None:
			# Split at tabs
			lineElements = line.strip().split("\t")
		else:
			# Split at the specified character
			lineElements = line.strip().split(options.splitCharacterPeakList)
